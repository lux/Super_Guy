...
void LoadingMenu::Draw(SDL_Renderer* rR) {
	if(loadingType) {
		...
		CCFG::getText()->Draw(rR, "MONDE", 320, 144);
		...
		CCFG::getText()->DrawCenterX(rR, "N'OUBLIEZ PAS QUE VOUS POUVEZ COURIR AVEC " + CCFG::getKeyString(CCFG::keyIDShift), 400, 16);
	} else {
		CCore::getMap()->DrawGameLayout(rR);
		CCFG::getText()->DrawCenterX(rR, "JEU TERMINE", 240, 16);
	}
}
